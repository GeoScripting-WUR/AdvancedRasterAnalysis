...
Figure below gives a simple demonstration of how the random forest method works in principle. For an introduction to the random forest algorithm, see this [presentation](http://www.slideshare.net/0xdata/jan-vitek-distributedrandomforest522013).
...
## Install igraph package if not installed
...
The remaining Landsat bands for the Gewata area are available as a [zip file](https://github.com/GeoScripting-WUR/AdvancedRasterAnalysis/raw/gh-pages/data/GewataBands.zip) from the data folder of this lesson's repository:
...
> Now build a `lm()` model in the same way as you build the randomForest model in 
 [VectorRaster lesson](http://geoscripting-wur.github.io/VectorRaster/)
...
- Produce one or more plots that demonstrate the relationship between the Landsat bands and the VCF tree cover. What can you conclude from this/these plot(s)? Based on the correlation, select suitable variables for the regression model. 
...
- Compute the RMSE between your predicted and the original tree cover values ([hint](http://stackoverflow.com/questions/17703037/how-to-perform-rmse-in-r))

**Bonus question:**

- Are the differences between the predicted and original tree cover the same for all of the 3 classes we used for the Random Forest classfication? Using the training polygons from the random forest classification, calculate the RMSE separately for each of the classes and compare. *Hint* - see `?zonal()`.

## Submission: Lesson 09
