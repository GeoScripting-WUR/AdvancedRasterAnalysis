...
## Make a new RasterBrick of covariates by 'bricking' together the existing gewata brick and NDVI and VCF layers
covs <- brick(c(gewata, ndvi, vcfGewata))
plot(covs)
```
...
names(classes) <- "class"
trainingbrick <- brick(c(covmasked, classes))
plot(trainingbrick)
```
