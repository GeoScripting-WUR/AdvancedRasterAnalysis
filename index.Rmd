...
Today you should exercise with the tutorial to work in R Markdown and for the exercise you have to create a html with the R Markdown file. The resulting HTML file then has to be uploaded to the website manually, or via git.
...
> **Question 3**: Assuming there are no training data available, is there a way we could improve the K-means classification performed in this example? Which of the two methods, randomforest and K-means, is computationally faster (hint: try the `system.time()` function)?
...
Publish a documented report to RPubs by using RMarkdown (.rmd file), copy paste the link to RPubs in your GitLab project and upload your .rmd to GitLab.
